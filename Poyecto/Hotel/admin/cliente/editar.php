<?php 
include '../../includes/header.php';
include '../../includes/database.php';

$id = $_GET['id'];

$q = oci_parse($conn, "SELECT * FROM TB_CLIENTE WHERE ID_CLIENTE = :id");
oci_bind_by_name($q, ":id", $id);
oci_execute($q);

$cliente = oci_fetch_assoc($q);
?>

<div class="container col-md-6 mt-4">

<h2 class="fw-bold mb-3">Editar Cliente</h2>

<?php
if ($_POST) {
    $sql = "BEGIN PKG_CLIENTE.UPD_CLIENTE(:id, :n, :ap1, :ap2, :c, :t, :tipo, 1); END;";
    $stid = oci_parse($conn, $sql);

    oci_bind_by_name($stid, ":id", $id);
    oci_bind_by_name($stid, ":n", $_POST['nombre']);
    oci_bind_by_name($stid, ":ap1", $_POST['ap1']);
    oci_bind_by_name($stid, ":ap2", $_POST['ap2']);
    oci_bind_by_name($stid, ":c", $_POST['correo']);
    oci_bind_by_name($stid, ":t", $_POST['telefono']);
    oci_bind_by_name($stid, ":tipo", $_POST['tipo_cliente']);

    oci_execute($stid);

    echo "<div class='alert alert-success'>Cliente actualizado correctamente.</div>";
}
?>

<form method="POST" class="card p-4 shadow">

    <label>Nombre:</label>
    <input type="text" name="nombre" class="form-control" value="<?= $cliente['NOMBRE'] ?>">

    <label class="mt-2">Primer apellido:</label>
    <input type="text" name="ap1" class="form-control" value="<?= $cliente['PRIMER_APELLIDO'] ?>">

    <label class="mt-2">Segundo apellido:</label>
    <input type="text" name="ap2" class="form-control" value="<?= $cliente['SEGUNDO_APELLIDO'] ?>">

    <label class="mt-2">Correo:</label>
    <input type="email" name="correo" class="form-control" value="<?= $cliente['CORREO_ELECTRONICO'] ?>">

    <label class="mt-2">Teléfono:</label>
    <input type="text" name="telefono" class="form-control" value="<?= $cliente['NUM_TELEFONO'] ?>">

    <label class="mt-2">Tipo de cliente (ID):</label>
    <input type="number" name="tipo_cliente" class="form-control" value="<?= $cliente['ID_TIPO_CLIENTE'] ?>">

    <button class="btn btn-warning mt-3">Actualizar</button>

</form>
</div>

<?php include '../../includes/footer.php'; ?>
